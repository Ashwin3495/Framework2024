...
//	LoginPage lp;	//Cannot be access since access modifier is default - accessed only within the same package
	...
	@Parameters({"browser_test"})	//value received from testNG will be passed as parameter in setup method to browsername
	@BeforeTest
	public void setup(@Optional("chrome") String browsername) throws Exception {  //  we cannot pass browser as it is string 

		df = new DriverFactory(); 	// object creation
		pf = df.initProp(); 		// Initialize property file to get the browser name
		
		if(browsername!=null) {
			pf.setProperty("browser", browsername);		// Set browser (make sure key matches exactly!)  name given in config file must be mentioned
		}
...
//			log.info("---screenshot is taken---");
		...
		//ChainTestListener.embed(DriverFactory.getScreenshotFile(), "image/png");      // get the screenshot for every test case irrespective of pass or fail
